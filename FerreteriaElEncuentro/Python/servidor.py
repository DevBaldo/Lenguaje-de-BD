from flask import Flask, render_template, request, jsonify, send_from_directory, redirect, url_for, flash, session, g
import cx_Oracle
import random
import os
import bcrypt
...
def facturacion():
    ...
    cursor.execute("""
        ...
        FROM 
            Facturacion F
        JOIN 
            Clientes C ON F.numero_factura = C.numero_factura
        JOIN 
            Productos P ON F.cod_producto = P.cod_producto
    """)
...
def inventario():
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute("SELECT cod_inventario, cod_sucursal, cantidad, fechaingreso FROM inventarios")
...
@app.route('/eliminarInventario/<codigo>', methods=['POST'])
def eliminar_inventario(codigo):
    connection = get_db_connection()
    cursor = connection.cursor()

    try:
        
        cursor.callproc('eliminar_inventario', [codigo])
        connection.commit()
    except cx_Oracle.DatabaseError as e:
        error, = e.args
        print(f"Error al eliminar inventario: {error.message}")
        connection.rollback()
    finally:
        cursor.close()
        connection.close()
...
@app.route('/addInventario', methods=['GET', 'POST'])
def add_inventario():
    if request.method == 'GET':
        return render_template('addInventario.html')

    if request.method == 'POST':
        cod_sucursal = request.form['cod_sucursal']
        ...
        connection = get_db_connection()
        cursor = connection.cursor()
        cursor.callproc('insertar_inventario', [codigo, cod_sucursal, cantidad, fecha_ingreso])
        connection.commit()
        cursor.close()
        connection.close()
...
def get_inventarios_eliminados():
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute("SELECT audit_id, cod_inventario, cod_sucursal, cantidad, fechaingreso, audit_timestamp, audit_operation FROM Inventarios_Audit ORDER BY audit_timestamp DESC")
    inventarios = cursor.fetchall()
    cursor.close()
    connection.close()
    return inventarios

@app.route('/verInventariosEliminados')
def ver_inventarios_eliminados():
    connection = get_db_connection()
    cursor = connection.cursor()

    cursor.execute("SELECT cod_inventario, cod_sucursal, cantidad, TO_CHAR(fechaingreso, 'YYYY-MM-DD'), TO_CHAR(audit_timestamp, 'YYYY-MM-DD HH24:MI:SS') FROM Vista_Inventarios_Eliminados")
    inventarios_eliminados = cursor.fetchall()
    
    cursor.close()
    connection.close()
    return render_template('ver_inventarios_eliminados.html', inventarios=inventarios_eliminados)


from datetime import datetime

@app.route('/updateInventario/<codigo>', methods=['GET', 'POST'])
def update_inventario(codigo):
    connection = get_db_connection()
    cursor = connection.cursor()

    if request.method == 'GET':
        cursor.execute("SELECT cod_sucursal, cantidad, TO_CHAR(fechaingreso, 'YYYY-MM-DD') FROM inventarios WHERE cod_inventario = :1", (codigo,))
        inventario = cursor.fetchone()
        cursor.close()
        connection.close()
        return render_template('updateInventario.html', inventario=inventario)

    elif request.method == 'POST':
        cod_sucursal = request.form['cod_sucursal']
        cantidad = request.form['cantidad']
        fecha_ingreso_str = request.form['fechaingreso']

        # Convertir la cadena de fecha en un objeto datetime
        fecha_ingreso = datetime.strptime(fecha_ingreso_str, '%Y-%m-%d')

        cursor.callproc('actualizar_inventario', [codigo, cod_sucursal, cantidad, fecha_ingreso])
        connection.commit()
        cursor.close()
        connection.close()
        return redirect('/inventario')
...
@app.route('/verStockProductos')
def ver_stock_productos():
    connection = get_db_connection()
    cursor = connection.cursor()

    cursor.execute("SELECT cod_producto, nombre, stock FROM Productos")
    productos = cursor.fetchall()

    cursor.close()
    connection.close()
    return render_template('ver_stock_productos.html', productos=productos)
...
def register():
    email = request.form['email']
    contrasena_plaintext = request.form['contrasena']
    hashed_password = bcrypt.hashpw(contrasena_plaintext.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')

    conn = get_db_connection()
    cursor = conn.cursor()
    
    cursor.execute('SELECT COUNT(*) FROM Usuarios WHERE email = :email', {'email': email})
    ...
    cursor.execute(
        'INSERT INTO Usuarios (id_usuario, email, contrasena) VALUES (usuarios_seq.NEXTVAL, :email, :contrasena)',
        {'email': email, 'contrasena': hashed_password}
    )
    conn.commit()
    cursor.close()
    conn.close()
